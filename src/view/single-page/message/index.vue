  ...
  methods: {
    ...
    handleView (msg_id) {
      ...
      this.getContentByMsgId({ msg_id }).then(content => {
        ...
        if (this.currentMessageType === 'unread') {
          this.hasRead({ msg_id }).catch(error => {
            this.$Message.error('无法读取信息' + error)
          })
        }
    ...
    removeMsg (item) {
      ...
      if (this.currentMessageType === 'readed') {
        this.removeReaded({ msg_id }).catch(error => {
          this.$Message.error('无法将记录删除' + error)
          item.loading = false
        })
      } else {
        this.restoreTrash({ msg_id }).catch(error => {
          this.$Message.error('无法恢复未删除记录' + error)
          item.loading = false
        })
      }
    }
